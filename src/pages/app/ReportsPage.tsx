...
const plannedFeatures = [
  {
    titleKey: 'reports.valueReport',
    descKey: 'reports.valueReportDesc',
    icon: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z',
  },
  {
    titleKey: 'reports.depreciation',
    descKey: 'reports.depreciationDesc',
    icon: 'M13 17h8m0 0V9m0 8l-8-8-4 4-6-6',
  },
  {
    titleKey: 'reports.serviceCosts',
    descKey: 'reports.serviceCostsDesc',
    icon: 'M11.42 15.17L17.25 21A2.652 2.652 0 0021 17.25l-5.877-5.877M11.42 15.17l2.496-3.03c.317-.384.74-.626 1.208-.766M11.42 15.17l-4.655 5.653a2.548 2.548 0 11-3.586-3.586l6.837-5.63m5.108-.233c.55-.164 1.163-.188 1.743-.14a4.5 4.5 0 004.486-6.336l-3.276 3.277a3.004 3.004 0 01-2.25-2.25l3.276-3.276a4.5 4.5 0 00-6.336 4.486c.091 1.076-.071 2.264-.904 2.95l-.102.085m-1.745 1.437L5.909 7.5H4.5L2.25 3.75l1.5-1.5L7.5 4.5v1.409l4.26 4.26m-1.745 1.437l1.745-1.437m6.615 8.206L15.75 15.75M4.867 19.125h.008v.008h-.008v-.008z',
  },
  {
    titleKey: 'reports.assignments',
    descKey: 'reports.assignmentsDesc',
    icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z',
  },
  {
    titleKey: 'reports.dataExport',
    descKey: 'reports.dataExportDesc',
    icon: 'M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3',
  },
  {
    titleKey: 'reports.serviceSchedule',
    descKey: 'reports.serviceScheduleDesc',
...
export function ReportsPage() {
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-light mb-1">{t('reports.title')}</h1>
      ...
      <div className="border border-border dark:border-border/50 relative overflow-hidden dark:glow-mixed">
        ...
        <div className="relative py-16 px-6 text-center">
          ...
          <h2 className="text-2xl font-light mb-3 text-gradient">{t('reports.moduleInDevelopment')}</h2>
          <p className="text-muted-foreground max-w-lg mx-auto">
            {t('reports.moduleDescription')}
          </p>
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-light">{t('reports.plannedFeatures')}</h2>
          <p className="text-sm text-muted-foreground mt-1">{t('reports.plannedFeaturesSubtitle')}</p>
        </div>
        <div className="text-xs text-muted-foreground border border-border dark:border-border/50 px-3 py-1.5">
          {plannedFeatures.length} {t('reports.featuresPlanned')}
        </div>
      </div>
          ...
          >
            ...
            <div className="mt-4 pt-4 border-t border-border dark:border-border/50 flex items-center justify-between">
              <span className="text-xs text-muted-foreground font-mono">
                {t('reports.feature')} #{String(index + 1).padStart(2, '0')}
              </span>
      ...
      <div className="border border-border dark:border-orange/20 p-6 bg-foreground/[0.02] dark:bg-orange/5">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <h3 className="font-medium mb-1">{t('reports.haveSuggestions')}</h3>
